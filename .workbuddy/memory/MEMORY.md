...
- **「经营概览」** `GET /v1/studio/overview`：
  ...
  - **待办事项页** `Modules/Studio/StudioTodoViewController.swift`（概览页「全部 ›」push 它，不是占位页）：3 行（退款审核 / 待打款确认 / 订单结算），条数取 `todos.pending_refunds / pending_payouts / pending_settle_orders`，角标红底；前两行进 `StudioRefundViewController(initialStatus: 0/1)`，结算行只 toast（Web 处理）
  - ⚠️ **cell 高度必须被约束「撑开」**：若卡片只钉 `top/bottom = contentView ± n`，而内容全用 `centerY` / 相对定位，就没有任何约束决定卡高 → `automaticDimension` 算出塌缩行高，**多行挤在一起、卡片高度 0 不可见**（内容溢出到卡外却仍可见）。给卡片一个显式高度（如 76）或用 `top/bottom` 链条把内容串起来。
  - ⚠️ **定位类约束别用 `lessThanOrEqualTo`**：角标 `trailing.lessThanOrEqualTo(...)` 会让横向位置欠定 → 渲染飘到卡片中间（实测 x≈194，应在 x≈346）。要定位就用等式，不等式只用于给文字留上界。
