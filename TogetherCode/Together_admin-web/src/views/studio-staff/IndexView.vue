...
import { fetchStudioStaff, createStudioStaff, toggleStudioStaffStatus, type StaffItem } from "../../services/studio";
...
const keyword = ref("");
const isOwner = auth.account?.role === "studio_owner";

async function loadData() {
  loading.value = true;
  try {
    const data = await fetchStudioStaff({ q: keyword.value.trim() || undefined, page: page.value, page_size: pageSize.value });
...
async function onToggle(row: StaffItem) {
  ...
  try {
    await ElMessageBox.confirm(
      next === 0 ? `停用后「${row.username}」将无法登录本店后台，确认停用？` : `确认启用「${row.username}」？`,
      next === 0 ? "停用账号" : "启用账号",
      { type: "warning", confirmButtonText: "确认", cancelButtonText: "取消" }
    );
    await toggleStudioStaffStatus(row.admin_id, next as 0 | 1);
  ...
  <div class="page-stack">
    <el-card v-loading="loading" shadow="never">
      <template #header>
        <div class="panel-header">
          ...
          <div>
            ...
            <el-button v-if="isOwner" type="primary" :icon="Plus" @click="openCreate">新增员工</el-button>
          </div>
        </div>
      </template>
      ...
      <el-alert
        ...
        :closable="false"
        show-icon
        title="员工账号可登录本工作室后台，拥有与本店相同的管理权限。只有店主可以新增或停用员工。"
      ...
      <el-table :data="list">
        ...
        <el-table-column label="操作" width="130" fixed="right">
          <template #default="{ row }">
            ...
            <span v-else-if="row.username === auth.account?.username" class="muted">当前登录</span>
    ...
    <el-dialog v-model="createVisible" title="新增本店员工" width="440px">
